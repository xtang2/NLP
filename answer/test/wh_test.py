...
class Wh_Answer:
    ...
    def getNumLevels(self,t):
        i=0
        check = ['placeholder']
        while(len(check) > 0):
            check = self.getAtLevel(i,t)
            i += 1
        return i-1

    def getLevelNumber(self,bigt,subt):
        for i in range(self.getNumLevels(bigt)):
            currLevel = self.getAtLevel(i,bigt)
            if subt in currLevel:
                return i
        return -1 #in case we can't find the smallt
